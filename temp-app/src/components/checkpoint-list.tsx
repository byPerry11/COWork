...
function SortableCheckpointItem({ checkpoint, onSelect, userRole, members }: SortableCheckpointItemProps) {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    if (checkpoint.is_completed && checkpoint.completed_by) {
        const completer = members.find(m => m.user_id === checkpoint.completed_by);
        if (completer && completer.member_color) {
            const color = completer.role === 'admin' ? '#a855f7' : completer.member_color;
            cardStyle = {
                borderColor: color,
                    ...
                    <div className="flex-1 flex items-center justify-between">
                        ...
                        {checkpoint.is_completed ? (
                            <CheckCircle2
                                ...
                                style={{ color: cardStyle['borderColor' as keyof typeof cardStyle] }}
                            />
                        ) : (
                        ...
                        <Sheet>
                            <SheetTrigger asChild>
                                <Button
                                    ...
                                    style={checkpoint.is_completed ? { borderColor: cardStyle['borderColor' as keyof typeof cardStyle], color: cardStyle['borderColor' as keyof typeof cardStyle] } : {}}
                                >
...
export function CheckpointList({ projectId, userRole, members }: CheckpointListProps) {
